...
class data_receipts:
    ...
    def save_as_csv(self, r, b, n = ''):
        ...
        bucket = b.replace('s3://', '').split('/')[0]
        prefix = '/'.join(b.replace('s3://', '').split('/')[1:])
        key = f"{prefix}/{self.business}/{csv_file}" if prefix else f"{self.business}/{csv_file}"
    ...
    def get_data(self):
        ...
        if self.business == 'eats':
            if "Thanks for ordering" in text_plain:
                receipts['amount_Charged']=self._extract_money(text_plain,"Amount Charged")
                receipts['total']=self._extract_money(text_plain,"Total")
                receipts['subtotal']=self._extract_money(text_plain,"Subtotal")
                receipts['delivery_fee']=self._extract_money(text_plain,"Delivery Fee")
                receipts['service_Fee']=self._extract_money(text_plain,"Service Fee")
                receipts['change']="0"
                receipts['restaurant']=self._extract_value(text_plain,"You ordered from")
                receipts['picked_up_from']=""
                receipts['delivered_to']=self._extract_value(text_plain,"Delivered to")
                receipts['lat_from']=0.0
                receipts['long_from']=0.0
                receipts['lat_to']=0.0
                receipts['long_to']=0.0
                receipts['items']=self.id
                items=[{'item':'Sample Meal','qty':1,'cost':receipts['subtotal'],'id':self.id}]
            ...
            if "Thanks for riding" in text_plain:
                receipts['amount_charged']=self._extract_money(text_plain,"Amount Charged")
                receipts['total']=self._extract_money(text_plain,"Total")
                receipts['subtotal']=receipts['total']
                receipts['booking_fee']=self._extract_money(text_plain,"Booking Fee")
                receipts['government_contribution']=self._extract_money(text_plain,"Government Contribution")
                receipts['wait_time']="0"
                receipts['trip_fare']=self._extract_money(text_plain,"Trip Fare")
                receipts['discounts']="0"
                receipts['before_Taxes']=receipts['trip_fare']
                receipts['balance']="0"
                receipts['time_payment']="0"
                receipts['distance']="0"
                receipts['unsettled_past_uber_trip']="0"
                receipts['distance_service']="0"
                d=self.toLocalDate(self.eml["header"]["date"])
                receipts['time_from_service']=d
                receipts['time_to_service']=d
                receipts['from_address']=self._extract_value(text_plain,"Pickup")
                receipts['to_address']=self._extract_value(text_plain,"Dropoff")
                receipts['lat_from']=0.0
                receipts['long_from']=0.0
                receipts['lat_to']=0.0
                receipts['long_to']=0.0
    ...
    def _extract_money(self, text, label):
        m = re.search(rf"{re.escape(label)}\s*MX?\$?\s*([0-9]+(?:\.[0-9]+)?)", text, re.IGNORECASE)
